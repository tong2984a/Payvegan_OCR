...
name = "python/payvegan_cases/case2b/qrcode.names"
cfg = "python/payvegan_cases/case2b/qrcode-yolov3-tiny.cfg"
weights = "python/payvegan_cases/case2b/qrcode-yolov3-tiny.weights"
...
def postprocess(frame, outs):
    ...
    for i in indices:
        ...
        if cv.waitKey(1) & 0xFF == ord('s'):
            img_path = filedir+str(numfile+1)+".jpg"
            cv.imwrite(img_path, cropped)
            f = open("python/payvegan_cases/case2b/result.txt", "a")
            data = decode(Image.open(img_path))
            f.write("QR code:\t"+img_path+"\n")
